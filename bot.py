...
async def approve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(user_id, "🎉 تم قبولك")
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "u1":
        ...
        keyboard = [
            [InlineKeyboardButton("تعليل", callback_data="u1_taaleel")],
    ...
    if data == "start_quiz":
        users.update_one(
            {"_id": user_id},
            {"$set": {"category": "u1_taaleel", "q_index": 0}}
        )
    ...
    if data.isdigit():
        ...
        users.update_one({"_id": user_id}, {"$inc": {"q_index": 1}})
